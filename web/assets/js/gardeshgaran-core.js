...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function isAnyFilterActive() {
    return selectedDays.size > 0 || selectedAirlines.size > 0 || realMin > REAL_MIN || realMax < REAL_MAX
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  priceOptions.forEach((option) => {
    option.addEventListener('click', () => {
      ...
      if (currentPriceFilter === selectedFilter) {
...
function uploadDocumentRequest(args) {
  ...
  const captcha = document.querySelector('#request-form').querySelector("#captchaContainer input[name='captcha']").value
